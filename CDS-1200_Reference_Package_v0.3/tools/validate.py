...
"""CDS-1200 v0.3 reference validation runner.
...
def sem_relationship_integrity(docs):
    """CDS200-R040: product targets resolve; CDS200-R045: no supersession cycles."""
    products = {d["product_id"]: d for d in docs if d["document_type"] == "product"}
    edges = {}
    for pid, product in products.items():
        for rel in product.get("relationships", []):
            if rel["target_kind"] == "product":
                tid = rel["target"]["document_id"]
                if tid not in products:
                    return f"relationship {rel['relationship_id']!r} targets unknown product {tid!r}"
                if rel["relationship_type"] in ("superseded_by", "replaces"):
                    edges.setdefault((rel["relationship_type"], pid), set()).add(tid)
    for rtype in ("superseded_by", "replaces"):
        graph = {pid: edges.get((rtype, pid), set()) for pid in products}
        ...
        def visit(node):
            if node in done:
                return None
            if node in visiting:
                return f"{rtype} cycle through {node!r}"
            visiting.add(node)
...
SEMANTIC_CHECKS = {
    ...
    "SEM-005": sem_relationship_integrity,
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CDS-1200 v0.3 reference fixtures")
