...
Rebuilt for the CDS v0.3 Review Draft (industry-profile expansion; owner decision IP3 promoting product relationships into the core model, ADR-D6).

- New schema `core/product-relationship.schema.json` (0.3.0): the CDS-200 §7A.1 record — governed `relationship_type`, `direction`, `target_kind` with a per-kind target shape (entity reference, standard, model family, application, external part or product), quantity, qualifiers, lifecycle status, validity window, provenance with evidence class, version.
- `core/product.schema.json` 0.2.0 → 0.3.0: `relationships` items are now product relationship records instead of bare entity references (breaking for that array; pre-1.0 minor bump recorded as open flag IP11). Every other product property is unchanged.
- `common/envelope.schema.json` 0.2.0 → 0.3.0: `cds_version` const is `"0.3"`; every fixture re-pinned.
- Fixtures: two positive homewares products exercising `requires` (standard target), `superseded_by` and `replaces` (product targets) and `compatible_with` (external product target); one negative product with an ungoverned `relationship_type` and no provenance.
- Semantic test SEM-005: product targets resolve within the document set and `superseded_by` / `replaces` chains are acyclic (CDS200-R040, CDS200-R045).
- Runner: package version 0.3.0; catalogue now 32 structural and 5 semantic cases (37/37 expected).
- Manifest regenerated; evidence report regenerated in the recorded environment.
