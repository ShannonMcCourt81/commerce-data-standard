# CDS-1200 Reference Package v0.3

This package accompanies CDS-1200 - Reference Implementation, Schema Package and Test Fixtures (v0.3 Review Draft).

## Contents

- `schemas/` - JSON Schema Draft 2020-12 reference schemas (22). Every schema carries a release-independent `$id` (`urn:cds:schema:<domain>:<name>`) and its own semver `version` keyword (ADR-D5).
- `fixtures/positive/` - documents that MUST validate (18).
- `fixtures/negative/` - documents that MUST fail validation, each with declared `expected_error_substrings` (14).
...
- executes the semantic tests (SEM-001..SEM-005), and
