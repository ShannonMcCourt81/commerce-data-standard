...
| Release | CDS v0.3 (single corpus release per ADR-D5; changelog REVIEW-021) |
| Date | 2026-08-04 |
...
| Product Relationship | A governed, typed, directed or symmetric link from a Product or Variant to another product, a model family, a standard, an application or an external part; kits and bundles are Products composed through `includes` relationships. | CDS-200 §7A |
...
## 7A. Product Relationships and Composition *(normative — added 2026-09-20 on owner decision IP3; ADR-D6)*

Products relate to one another: an accessory fits a device, a part fits a vehicle, a kit includes its components, a model supersedes its predecessor, two products are equivalent. Those facts are the highest-consequence data in several verticals (a wrong fitment is a safety and returns event) and the most often faked (a "compatible" tag). The canonical model therefore treats every such fact as a **product relationship record**: a governed, directed or symmetric, typed link carried in the product's or variant's `relationships` collection with the same evidence, lifecycle and verification obligations as any other governed fact. Industry profiles add the vertical-specific target vocabularies and constraints (for example fitment against a declared vehicle vocabulary in CDS-1900 §6) without redefining the record.

### 7A.1 The Product Relationship Record *(normative)*

| Property | Meaning | Requirement |
|---|---|---|
| relationship_id | Stable identifier | Required |
| source | The Product or Variant the record belongs to (implicit when embedded in that document's `relationships` collection) | Required |
| relationship_type | Governed value from the `relationship_type` dictionary (reference package Chapter 18): `fits`, `compatible_with`, `accessory_for`, `requires`, `includes`, `part_of_set`, `replaces`, `superseded_by`, `alternative_to`, `cross_reference`, `successor_of`, `variant_family_of`, or a governed organisation extension | Required |
| direction | `directed` or `symmetric`, as declared for the type in the dictionary | Required |
| target_kind | `product`, `variant`, `model_family`, `standard`, `application`, `external_part`, `external_product` | Required |
| target | A reference appropriate to the kind: a catalogue entity reference (product, variant), a governed standard value, a model-family reference, an application record (CDS-1900 §6), or an external part or product record carrying the issuing scheme and identifier | Required |
| quantity | Quantity of the target per source (`includes`, `requires`) or of the source per application (`fits`) | Conditional |
| qualifiers | Governed qualifier values (position, side, basis of equivalence, notes from a governed vocabulary) | Optional |
...
| provenance | Source, evidence class (CDS-700 §7), actor and timestamps (CDS-400 §23) | Required |
| version | Record version (§14) | Required |

The reference package carries this record as the component schema `urn:cds:schema:core:product-relationship` (CDS-1100 §8; CDS-1200).

**CDS200-R037** Every product relationship MUST be a record conforming to §7A.1, carried in the product's or variant's `relationships` collection or an equivalent governed structure, with a governed relationship type, a declared direction, a typed target and provenance.

**CDS200-R038** Relationship types MUST be governed dictionary values whose direction (directed or symmetric) is declared in the dictionary; an implementation MUST NOT invent relationship semantics per product.

**CDS200-R039** Relationships MUST NOT be stored as tags, in free-text fields, in titles or in category membership as the system of record; such projections MAY be generated from relationship records.

**CDS200-R040** A relationship whose target is another product or variant in the catalogue MUST reference that entity's stable identifier; a relationship whose target is outside the catalogue (a standard, a vehicle, an external part number) MUST use the external target kinds with the identifying scheme declared.

**CDS200-R041** Relationships MUST carry provenance and evidence class (CDS-700 §7); an AI-proposed relationship MUST be a proposal with evidence and MUST NOT be published without human review (CDS-700 §19).

**CDS200-R042** Relationships are versioned governed facts (§14): changes follow the versioning, deprecation-with-replacement and breaking-change patterns of CDS-400 §24, and a superseded relationship MUST be retained with its validity window, never deleted.

**CDS200-R043** Symmetric relationships MUST be stored once and evaluated in both directions; directed relationships MUST NOT be assumed to hold in reverse.

**CDS200-R044** Where a channel accepts relationship data (compatibility lists, complementary products, fitment tables, bundle components), the projection MUST be generated from relationship records, declared in the Field Mapping with its comparison strategy and verified under CDS-500; where the channel exposes no read-back for relationships, the verification status MUST be UNOBSERVABLE, not MATCH.

**CDS200-R045** Cycles in `superseded_by` and `replaces` chains MUST be rejected at validation.

### 7A.2 Composition: Kits, Bundles and Assemblies *(normative)*

A kit is a Product whose sellable content is composed of other Products. The model distinguishes a **stocked kit** (assembled in advance, with its own identifier and inventory), a **virtual bundle** (assembled at order time from component inventory) and an **assembly** (a Product whose components are also sold separately as parts).

**CDS200-R046** A kit, bundle or assembly MUST be a Product whose composition is expressed as `includes` relationship records with quantities; component identity, attributes, compliance records and declarations remain on the components.

**CDS200-R047** The kit kind (stocked kit, virtual bundle, assembly) MUST be a governed attribute, because inventory authority, trade identifiers and channel projection differ: a stocked kit MAY carry its own trade identifier; a virtual bundle MUST NOT claim a trade identifier it does not have.

**CDS200-R048** Kit-level declarations that depend on components (allergen, ingredient, nutrition, dangerous-goods, battery and similar declarations as defined by the applicable industry profiles) MUST be derived from the component records under declared rules and MUST NOT be independently authored.

**CDS200-R049** Where a channel supports bundle semantics (a bundle flag, component listing, bundle pricing), the projection MUST be generated from the relationship records and declared in the Field Mapping; availability of a virtual bundle MUST be derived from component availability under the inventory authority and MUST be verifiable (CDS-500).

**CDS200-R050** Removing or replacing a component MUST be a governed relationship change (R042) with the kit's revision advanced (§14), so that a published kit state can be traced to its composition.
