...
| Profile identifiers | `cds.profile.sports_outdoor.v0_3`, `cds.profile.toys_games.v0_3`, `cds.profile.pets.v0_3` (registered in CDS-1500 §2.1) |
...
**CDS2000-R007** Kits, sets and bundles (a tent with footprint, a game with expansion, a starter aquarium) MUST follow CDS-200 §7A.2; batteries and rechargeable products MUST follow CDS-1800 §8 and CDS-1900 §12.

**CDS2000-R008** Included and required-but-not-included items (batteries, balls, mounting hardware, food bowls) MUST be `includes` and `requires` relationship records (CDS-200 §7A) so that "batteries not included" and similar statements derive from data.
...
| Sports & Outdoor | Cycling (bikes, components) | brand; MPN; frame size system and size; wheel size; compatibility records (CDS-200 §7A); weight | groupset, gearing, geometry as typed values; e-bike battery and motor per CDS-1800 | frame size, colour |
...
| Relationship records | Compatibility (components, fin systems, mounts), included items | CDS-200 §7A |
...
**CDS2000-R013** Cycling and other component-based equipment MUST express component compatibility (frame, wheel size, drivetrain, mount standards) through relationship records or governed standard values per CDS-200 §7A and CDS-1900 §7; "fits most bikes" MUST be an explicit universal-fit declaration within a governed scope (CDS1900-R018).
...
**CDS2000-R037** Consumable compatibility (filter cartridges for a filter model, litter for a system, replacement parts for feeders) MUST be `compatible_with`, `accessory_for` or `fits` relationship records per CDS-200 §7A.
...
| Compatibility (components, consumables) | Candidate relationships with evidence | CDS-200 §7A review rules |
...
| Relationship records (`compatible_with`, `includes`, `requires`, `fits`) | Product | Records per CDS-200 §7A | C |
...
| Relationship records (`compatible_with`, `accessory_for`, `fits`) | Product | Records per CDS-200 §7A | C |
...
END OF CDS-2000 v0.3 REVIEW DRAFT
