...
| Companion package | CDS-1200 Reference Package v0.3.0 (semver, independent of chapter prose — ADR-D5; schemas remain 0.2.0) |
...
```
CDS-1200_Reference_Package_v0.3/
  README.md
  CHANGELOG.md
  LICENSE.txt
  requirements.txt
...
The v0.3 package ships **22 schemas** (the 21 of v0.2 plus `core/product-relationship`, added on owner decision IP3) implementing the registry needed to demonstrate the canonical and publication lifecycle. Each schema declares JSON Schema Draft 2020-12, a release-independent URN `$id` and its own semantic version.
...
The v0.3 suite contains **18 positive structural fixtures** (the 16 of v0.2 plus two homewares products exercising relationship records). Together they demonstrate the canonical product, controlled dictionary, classification, channel profile and publication assurance chain.
...
The package includes **14 negative fixtures** (the 13 of v0.2 plus an invalid relationship record). Each fails under its declared schema, and — new in v0.2 — each catalogue entry declares the *intended* error via `expected_error_substrings`. Negative tests prove that a validator rejects invalid input for the right reason, not merely that it produced some error.
...
The following is the recorded outcome of executing Reference Package v0.3.0 in a clean environment with the pinned dependencies installed. It is an evidence snapshot, not a normative claim (§1); reproducing it requires the recorded environment or a compatible one.

| Metric | Result |
|---|---|
| Schemas | 22 (21 at v0.2 plus `core/product-relationship` 0.3.0; `core/product` and `common/envelope` at 0.3.0) |
| Positive fixtures | 18 |
| Negative fixtures | 14 |
| Structural cases | 32 / 32 as expected |
| Semantic cases (SEM-001..005) | 5 / 5 as expected |
| Total cases | 37 / 37, 0 unexpected outcomes |
| Manifest-listed files | 64 (manifest itself and `evidence/` excluded by convention, §6) |
| Evidence ties | `manifest_sha256` embedded in report; manifest `package_hash` present |

Recorded environment: Python 3.14.7; `jsonschema` 4.26.0, `referencing` 0.37.0, `rfc3339-validator` 0.1.4 (per `evidence/test-report.json`, generated 2026-09-20). In a clean environment *without* the pinned dependencies, the runner exits FATAL at startup rather than reporting a weakened pass (§5) — the failure mode that produced v0.1's non-reproducing 23/23 (REVIEW-010) can no longer occur silently.
...
**CDS1200-R039** Every package release MUST publish a changelog, a manifest and a test report. *(The v0.3.0 release ships all three; v0.2.0 first closed finding CDS1200-5, under which the v0.1 release failed its own rule.)*
...
unzip CDS-1200_Reference_Package_v0.3.zip
cd CDS-1200_Reference_Package_v0.3
