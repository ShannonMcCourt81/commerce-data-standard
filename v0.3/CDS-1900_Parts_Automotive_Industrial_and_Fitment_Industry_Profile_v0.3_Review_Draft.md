...
| Normative status | §1, §3, §6–§10, §12–§14, §19 and Appendix A are normative. §2, §5 and §11 (pointers), §4, §15–§18, §20, §21 and Appendices B–D are informative; Appendix D seeds the jurisdiction requirement register whose record structure and obligations are normative in CDS-1500 §2.2. Every table is individually marked. **This chapter is the single normative home (CDS000-R005) for the dangerous-goods declaration (§12).** The product relationship record and the composition rules were homed here in the 2026-09-20 draft and promoted to CDS-200 §7A on owner decision IP3 (ADR-D6); §5 and §11 are pointers carrying the retired-identifier mapping. The dangerous-goods declaration is a product-level transport and hazard declaration that does not depend on the relationship model, so it stays here. |
...
| Profile identifier | `cds.profile.parts_fitment.v0_3` (registered in CDS-1500 §2.1) |
...
CDS-1900 defines the industry profile for automotive and powersports parts and accessories, industrial and maintenance supplies, tools and hardware, electrical and plumbing components, hobby components (radio-control, model, craft hardware) and spare parts for appliances and equipment. It also defines, for the whole corpus, how dangerous goods are declared for transport (§12), and applies the corpus product relationship and composition model (CDS-200 §7A) to fitment, interchange, supersession and parts kits.
...
**CDS1900-R001** An implementation claiming the Parts and Fitment Profile MUST represent fitment, compatibility, inclusion, interchange and supersession as product relationship records (CDS-200 §7A), never as tags, free text, title conventions or category placement.
...
**CDS1900-R004** This profile MUST NOT weaken any rule of CDS-1500 that it reuses; where this chapter is silent, CDS-1500, CDS-1800 (for technical specifications), CDS-200 §7A (for relationships and composition) and the core chapters govern.
...
## 5. Product Relationship Record *(informative pointer — the record is defined in CDS-200 §7A)*

The product relationship record was first homed in this chapter in the 2026-09-20 draft and promoted to the core model on owner decision IP3 (ADR-D6). Its definition, the governed `relationship_type` dictionary binding and the generic rules now live in CDS-200 §7A.1 (CDS200-R037 to CDS200-R045); this chapter adds the fitment-specific target vocabulary and constraints (§6) and the interchange and supersession rules (§7). The identifiers CDS1900-R008 to CDS1900-R016 that carried the generic rules in the 2026-09-20 draft are retired with the replacement mapping below and are not reused (CDS000-R003); they remain listed in the requirements index (REVIEW-004) marked `retired` with their replacements so that citations stay discoverable, and they carry no requirement (owner decision IP12).

| Retired identifier | Replacement | Subject |
|---|---|---|
| CDS1900-R008 | CDS200-R037 | Record conformance, governed type, typed target, provenance |
| CDS1900-R009 | CDS200-R038 | Governed relationship types with declared direction |
| CDS1900-R010 | CDS200-R039 | No tags, free text, titles or categories as the system of record |
| CDS1900-R011 | CDS200-R040 | Catalogue targets by stable identifier; external target kinds |
| CDS1900-R012 | CDS200-R041 | Provenance, evidence class, human review of AI proposals |
| CDS1900-R013 | CDS200-R042 | Versioned governed facts; retention of superseded records |
| CDS1900-R014 | CDS200-R044 | Channel projection from records; UNOBSERVABLE where no read-back |
| CDS1900-R015 | CDS200-R043 | Symmetric relationships stored once |
| CDS1900-R016 | CDS200-R045 | Cycle rejection in supersession chains |

## 6. Fitment and Application Data *(normative)*

Fitment is a `fits` relationship (CDS-200 §7A) whose target is an application record: a description of the host (vehicle, machine, appliance model) drawn from a declared vocabulary. The corpus ships no vehicle or equipment tables — the leading reference databases are licensed (for example the Auto Care Association's vehicle configuration database used by the ACES standard, or manufacturer model lists) — so the organisation declares which vocabulary and release each application record uses.
...
**CDS1900-R023** Fitment published to a channel (a vehicle compatibility table, a fitment metafield, a "fits your vehicle" lookup) MUST be generated from fitment records and verified under CDS-500 where the channel exposes read-back; otherwise reported UNOBSERVABLE (CDS200-R044).
...
**CDS1900-R026** Supersession MUST be a directed `superseded_by` relationship with an effective date; a superseded part's lifecycle state MUST reflect its status and its record MUST be retained (CDS200-R042) so that searches for the old number resolve to the replacement.
...
**CDS1900-R028** Where a channel or marketplace uses interchange data for search, the projection MUST be generated from relationship records (CDS200-R044).
...
## 11. Kits, Bundles and Assemblies *(informative pointer — composition rules are defined in CDS-200 §7A.2)*

Kits, virtual bundles and assemblies are composed through `includes` relationship records under CDS-200 §7A.2 (CDS200-R046 to CDS200-R050). In this profile the composition rules are exercised by parts kits, service kits and assemblies whose components are also sold as spare parts; kit-level dangerous-goods status is derived from the components under §12 (R054). The identifiers CDS1900-R044 to CDS1900-R048 that carried the composition rules in the 2026-09-20 draft are retired with the replacement mapping below and are not reused (CDS000-R003); they remain listed in the requirements index marked `retired` with their replacements and carry no requirement (owner decision IP12).

| Retired identifier | Replacement | Subject |
|---|---|---|
| CDS1900-R044 | CDS200-R046 | Composition as `includes` records with quantities |
| CDS1900-R045 | CDS200-R047 | Governed kit kind; trade identifiers |
| CDS1900-R046 | CDS200-R048 | Kit-level declarations derived from components |
| CDS1900-R047 | CDS200-R049 | Bundle projection and availability |
| CDS1900-R048 | CDS200-R050 | Component changes advance the kit revision |
...
**CDS1900-R054** Kits inherit the most restrictive dangerous-goods status of their components under declared rules (CDS200-R048); a kit containing a regulated component MUST be declared regulated.
...
- a fitment held only in a tag or title, which must fail (R001, CDS200-R039);
...
- a `superseded_by` cycle that validation must reject (CDS200-R045);
...
- a kit containing a lithium-battery component declared "not regulated" at kit level (R054, CDS200-R048);
...
|---|---|---|---|
| relationship_type | 18 (new) | Product relationship records (CDS-200 §7A) | Bound (corpus-wide via CDS-200 §7A) |
...
END OF CDS-1900 v0.3 REVIEW DRAFT
