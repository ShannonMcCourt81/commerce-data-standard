...
## Shopify Store Setup Runbook (CDS v0.3)

| Field | Value |
|---|---|
| Status | **v0.3 Companion — Informative** (profile companion; not normative standard text; content carried forward from the v0.2 runbook with the §2.5 and profile-stack updates of 2026-09-20) |
| Release | CDS v0.3 (corpus release, 2026-09-20). The platform profile it applies, CDS-SHOPIFY-0.2, carries its own release number bound to the verification of its platform statements (2026-08-03/04; CDS-900 §1, ADR-D7) — the two are versioned separately |
| Date | 2026-08-17 |
...
| Profile stack | CDS Core → `cds.profile.apparel.v0_3` + `cds.profile.homewares.v0_3` (CDS-1500) → CDS-SHOPIFY-0.2 (CDS-900 §4; platform profile release 0.2, statements verified 2026-08-03/04, carried forward unchanged into corpus v0.3) → this store's organisation profile |
| Normative status | Entirely **informative**. Every step cites the requirement IDs it implements; the cited chapters govern. Where this runbook and a CDS v0.3 chapter disagree, the chapter wins. |
| Supersedes (operationally) | Legacy implementation runbook (its sequencing doctrine is retained; its v0.1-era details are re-based on CDS v0.2, carried into v0.3) |
...
The seed package (0.8.0) ships size systems (Chapter 5, including apparel, footwear, bedding, ring, bike-frame and head-circumference systems), fabric types (Chapter 3) and descriptive care instructions (Chapter 4). It does not ship: the trademarked care-symbol set, country of origin (generate from ISO 3166 at adoption), component roles, season/weather relevance, use environment. For launch you need **size systems** governed at minimum (apparel is unpublishable without them): adopt the Chapter 5 values as your governed size-system dictionary now and add any brand-specific systems (e.g. a numeric womenswear system with its sort keys); queue the rest. Until governed, these fields cannot satisfy R-level requirements. If the store sells into more than one country, also open the jurisdiction requirement register (CDS-1500 §2.2) from the Appendix G seed for apparel and homewares.
