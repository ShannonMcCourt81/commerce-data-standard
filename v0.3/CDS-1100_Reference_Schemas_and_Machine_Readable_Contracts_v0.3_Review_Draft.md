...
| Dependencies | CDS-000 through CDS-1000; reference schema package CDS-1200 v0.3.0 |
...
**Relationship to the shipped package.** Every schema this chapter describes exists in the CDS-1200 Reference Package v0.3.0 (`schemas/`, 21 schemas at schema version 0.2.0). Where this chapter states what a schema requires, the statement has been checked against the shipped schema text. The published specification governs where package and prose conflict (CDS-1200 README), but as of v0.2 no such conflict is known.
...
Platform and industry profiles are not schema directories in v0.2: platform-specific constraints live in CDS-900 profiles and industry constraints in CDS-1500 through CDS-2000 (register: CDS-1500 §2.1), applied by composition (§20). The product relationship record (CDS-200 §7A) is carried in the canonical product contract's `relationships` collection (§8) as the component schema `urn:cds:schema:core:product-relationship` (reference package 0.3.0; the product schema moves to 0.3.0 because its `relationships` items change from bare entity references to relationship records). The v0.1 `profiles/` directory sketch is withdrawn.
...
- **Reference package.** The CDS-1200 Reference Package v0.3.0 ships the 22 schemas of Appendix A (21 at v0.2 plus `core/product-relationship` at 0.3.0), positive and negative fixtures, the executed structural and semantic test catalogues, the validation runner and the regenerable manifest. Package composition, runner behaviour and evidence conventions are CDS-1200's subject matter.
...
END OF CDS-1100 v0.3 REVIEW DRAFT
