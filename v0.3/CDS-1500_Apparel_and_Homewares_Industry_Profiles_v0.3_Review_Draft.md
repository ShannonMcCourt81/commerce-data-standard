...
## CDS-1500 — Industry Profile Framework, Apparel and Homewares Profiles

| Field | Value |
|---|---|
| Status | **v0.3 Review Draft** (working source; not an approved standard) |
| Release | CDS v0.3 (single corpus release per ADR-D5; the 2026-09-20 industry-profile expansion of the v0.2 Review Draft, changelog REVIEW-021) |
...
| Former title | "Apparel and Homewares Industry Profiles and Reference Dictionaries" (2026-08-04 to 2026-09-20). Retitled on owner decision IP1 so that the chapter's corpus-wide role — the industry profile framework, the Industry Profile Register (§2.1) and the Jurisdiction Requirement Register (§2.2) — is explicit. The chapter number and file name are unchanged for citation stability; citations of the former title resolve to this chapter; Appendix E remains the dictionary package contract. |
...
| Profile identifiers | Base profiles `cds.profile.apparel.v0_3` and `cds.profile.homewares.v0_3`; extension profiles `cds.profile.footwear.v0_3`, `cds.profile.jewellery.v0_3` (§12), `cds.profile.furniture.v0_3`, `cds.profile.garden.v0_3` (§14A–§14B). All profile identifiers in the corpus are registered in §2.1 |
...
|---|---|---|---|---|---|
| `cds.profile.apparel.v0_3` | Apparel | CDS-1500 §5–§11 | Base | — | 1, 2, 3, 4, 5, 6 |
| `cds.profile.footwear.v0_3` | Footwear | CDS-1500 §12.1 | Extension | `cds.profile.apparel.v0_3` | 2, 5, 8 |
| `cds.profile.jewellery.v0_3` | Jewellery and accessories | CDS-1500 §12.2 | Extension | `cds.profile.apparel.v0_3` | 1, 9 |
| `cds.profile.homewares.v0_3` | Homewares | CDS-1500 §13–§17 | Base | — | 1, 2, 3, 4, 6 (styles), 7 |
| `cds.profile.furniture.v0_3` | Furniture | CDS-1500 §14A | Extension | `cds.profile.homewares.v0_3` | 2, 3, 5 (bedding sizes), 7, 15 |
| `cds.profile.garden.v0_3` | Garden and outdoor living | CDS-1500 §14B | Extension | `cds.profile.homewares.v0_3` | 2, 7, 16 |
| `cds.profile.beauty_health.v0_3` | Beauty, health and personal care | CDS-1600 | Base | — | 10, 17 (ingestibles), 18, 19 |
| `cds.profile.food_beverage.v0_3` | Food, beverage and grocery | CDS-1700 | Base | — | 17, 18, 19 |
| `cds.profile.electronics.v0_3` | Consumer electronics, appliances and technical goods | CDS-1800 | Base | — | 11, 18, 19 |
| `cds.profile.parts_fitment.v0_3` | Parts, automotive, industrial and fitment | CDS-1900 | Base | — | 18, 19 (11 where relevant) |
| `cds.profile.sports_outdoor.v0_3` | Sports and outdoor | CDS-2000 §5 | Base | — | 12, 5, 19 |
| `cds.profile.toys_games.v0_3` | Toys and games | CDS-2000 §6 | Base | — | 13, 19 |
| `cds.profile.pets.v0_3` | Pet supplies | CDS-2000 §7 | Base | — | 14, 17, 19 |

Cross-industry mechanisms homed in one chapter and cited by the others: the product relationship record, kits and bundles, and the dangerous-goods declaration (CDS-200 §7A and CDS-1900 §12); market registration and responsible-person records (CDS-1600 §12); ingredient, allergen, nutrition and net-quantity patterns (CDS-1700 §5–§9); typed technical specifications and per-market compliance records (CDS-1800 §6, §12); the jurisdiction requirement register (§2.2 below).
...
**CDS1500-R055** A claim of an extension profile MUST include a claim of the profile it requires; the extension's requirements add to, and MUST NOT weaken, the parent's. Conformance evaluation of an extension claim MUST apply the parent's requirements only as they apply to the extension's product families under the category-profile requiredness rule (R009), and MUST NOT impose requirements the parent declares for sibling product families (for example garment fibre composition on footwear, or cushion fill on garden tools).
...
| verification_status / verified_on / verified_against | `verified` (the primary instrument or a regulator publication was read), `candidate_retrieved` (a regulator page was retrieved but the instrument not read), `candidate_unverified` (seeded from secondary sources or general knowledge) or `not_seeded`; when; and against what. Only `verified` entries are operative (R086) | Required |
...
**CDS1500-R060** The seed tables in the industry profile chapters are informative starter content, dated and split into verified seed entries and research candidates: an organisation MAY adopt verified seed entries as the initial content of its register, at which point they become governed records under that organisation's control and change through CDS-800 governance; research candidates are governed by R086 and MUST NOT be adopted as obligations without verification against the primary instrument.
...
**CDS1500-R086** A seed entry marked retrieved, partially verified, unverified or not seeded is a research candidate: it MUST NOT be loaded into the operative register as an obligation or evaluated with a `block` or `warn` consequence, and MUST NOT be presented to adopters, auditors or channels as a verified regulatory requirement; an organisation MAY hold research candidates in a separately labelled candidate list with a `record` consequence only, and MAY promote a candidate to the operative register only after its verification against the primary instrument is recorded (`verified`, with verified_on and verified_against).
...
### 12.1 Footwear Extension Profile *(normative; `cds.profile.footwear.v0_3`, requires `cds.profile.apparel.v0_3`)*
...
### 12.2 Jewellery and Accessories Extension Profile *(normative; `cds.profile.jewellery.v0_3`, requires `cds.profile.apparel.v0_3`)*
...
## 14A. Furniture Extension Profile *(normative; `cds.profile.furniture.v0_3`, requires `cds.profile.homewares.v0_3`)*
...
| Relationship records | Modular and sectional configurations, matching pieces, replacement parts (CDS-200 §7A) | Product |
...
**CDS1500-R075** Modular and sectional furniture MUST express configurations, matching pieces and replacement parts through relationship records per CDS-200 §7A; a configurator's output MUST be traceable to those records.

## 14B. Garden and Outdoor Living Extension Profile *(normative; `cds.profile.garden.v0_3`, requires `cds.profile.homewares.v0_3`)*
...
**CDS1500-R082** An implementation claiming the CDS Footwear Profile MUST satisfy the Apparel Profile (R052) as applicable under R055, and R033, R062–R064 (size system and width as variant options with sort order, component materials, governed footwear dictionaries).

**CDS1500-R083** An implementation claiming the CDS Jewellery and Accessories Profile MUST satisfy the Apparel Profile (R052) as applicable under R055, and R065–R068 (metal, fineness, plating and stone as separate governed attributes; stone status; size systems; market marking and content obligations evaluated through the register).

**CDS1500-R084** An implementation claiming the CDS Furniture Profile MUST satisfy the Homewares Profile (R053) as applicable under R055, and R069–R075 (assembled and per-package dimensions, governed assembly and delivery classes, bedding size systems, governed mattress firmness, component materials with flammability and chemical statements as records, toppling warnings projected and verified, configurations as relationship records).

**CDS1500-R085** An implementation claiming the CDS Garden and Outdoor Living Profile MUST satisfy the Homewares Profile (R053) as applicable under R055, and R076–R080 (explicit outdoor suitability, governed plant naming and sun exposure with region-specific schemes, dangerous-goods and registration records for chemicals and gas products, biosecurity restrictions at preflight, CDS-1800 compliance for gas and electrical appliances).
...
| CDS-ADR-1500-009 | One corpus-wide industry profile register (§2.1); extension profiles require their parent, with conformance limited to the applicable product families (R055); new verticals get their own chapters (CDS-1600 through CDS-2000) selected by global market research (REVIEW-020); this chapter retitled "Industry Profile Framework, Apparel and Homewares Profiles" | Accepted (owner decisions IP1 override, IP8 accepted with clarification, 2026-09-20) |
| CDS-ADR-1500-010 | Regulatory obligations are not encoded in requirement levels; each organisation maintains a jurisdiction requirement register (§2.2) evaluated at preflight per market, seeded per chapter for AU, NZ, US, EU, GB and CA; seeds are split into verified seed entries and non-normative research candidates, and candidates are excluded from the operative register until verified (R086) | Accepted (owner decision IP9 modify, 2026-09-20) |
| CDS-ADR-1500-011 | The product relationship record and composition rules, homed in CDS-1900 in the 2026-09-20 draft, are promoted to CDS-200 §7A (ADR-D6); the dangerous-goods declaration stays in CDS-1900 §12 as a product-level declaration independent of the relationship model | Accepted (owner decision IP3 override, 2026-09-20) |
...
| Relationship records (configurations, matching pieces, parts) | Product | Records per CDS-200 §7A | C |
...
Conventions: obligation types are those of §2.2. Each jurisdiction is split into **verified seed entries** (the primary instrument or a regulator publication was read on 2026-09-20; adoptable into the operative register under R060) and **research candidates** (retrieved without reading the instrument, partially verified, seeded from general knowledge, or not seeded; non-normative and excluded from the operative register until verified against the primary instrument, R086). "Not seeded" means the organisation supplies the entry. Jurisdiction codes are those of the `jurisdiction` dictionary (package Chapter 19); scheme codes are those of `regulatory_scheme`. Cosmetics-like goods (fragrance, jewellery cleaners) follow CDS-1600 Appendix D; electrical and battery-powered goods (heated apparel, smart watches, powered furniture) follow CDS-1800 Appendix D; chemicals and gas products follow CDS-1900 Appendix D.

### G.1 Australia (AU)

#### G.1.1 Verified seed entries *(adoptable into the operative register under CDS1500-R060)*
...
#### G.1.2 Research candidates *(non-normative; excluded from the operative register until verified against the primary instrument, CDS1500-R086)*
...
#### G.2.1 Verified seed entries *(adoptable into the operative register under CDS1500-R060)*

*None in this release.*

#### G.2.2 Research candidates *(non-normative; excluded from the operative register until verified against the primary instrument, CDS1500-R086)*
...
#### G.3.1 Verified seed entries *(adoptable into the operative register under CDS1500-R060)*
...
#### G.3.2 Research candidates *(non-normative; excluded from the operative register until verified against the primary instrument, CDS1500-R086)*
...
#### G.4.1 Verified seed entries *(adoptable into the operative register under CDS1500-R060)*
...
#### G.4.2 Research candidates *(non-normative; excluded from the operative register until verified against the primary instrument, CDS1500-R086)*
...
#### G.5.1 Verified seed entries *(adoptable into the operative register under CDS1500-R060)*

*None in this release.*

#### G.5.2 Research candidates *(non-normative; excluded from the operative register until verified against the primary instrument, CDS1500-R086)*
...
#### G.6.1 Verified seed entries *(adoptable into the operative register under CDS1500-R060)*
...
#### G.6.2 Research candidates *(non-normative; excluded from the operative register until verified against the primary instrument, CDS1500-R086)*
