...
## Release history and old links

`RELEASES.md` lists every release with its date, status and git tag, and maps each superseded path to its current one. When a release is superseded its old folder keeps a `README.md` pointer and every old markdown and HTML path keeps a stub that names the replacement, so an old link resolves to a note rather than a dead end; the full old snapshot stays browsable at its tag (for example `v0.2-review-draft`). The pointer layer is generated by `tools/build_release_pointers.py` from `tools/release_history.json`; add the next release there before relabelling.
